...
class FRWeaponAttachment extends Actor
	dependson(FRPawn);
...
simulated function AttachTo(FRPawn P)
{
...
{
	local int i;
	local FRPhysicalMaterialProperty PhysicalProperty;
	
		// get physical material
		if(HitMaterial != none)
		{
			PhysicalProperty = FRPhysicalMaterialProperty(HitMaterial.GetPhysicalMaterialProperty(class'FRPhysicalMaterialProperty'));
		}
